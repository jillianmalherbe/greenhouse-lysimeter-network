			       ...
			       const struct bt_mesh_sensor_value *value)
{
	...
	} else if (sensor->id == bt_mesh_sensor_time_since_presence_detected.id) {
	...
	} else if (sensor->id == bt_mesh_sensor_present_dev_op_temp.id) {
		printk("Sensor Temperature: %s\n", bt_mesh_sensor_ch_str(value));
	} else if (sensor->id == bt_mesh_sensor_pressure.id) {
		printk("Sensor Pressure: %s\n", bt_mesh_sensor_ch_str(value));
	}
}
...
static void get_data(struct k_work *work)
{
	...
	case (1): {
		err = bt_mesh_sensor_cli_series_entries_get(
			&sensor_cli, NULL, &bt_mesh_sensor_rel_runtime_in_a_dev_op_temp_range, NULL,
			NULL, NULL, NULL);
		if (err) {
	...
	case (2): {
		err = bt_mesh_sensor_cli_get(&sensor_cli, NULL,
					     &bt_mesh_sensor_time_since_presence_detected, NULL);
		if (err) {
	...
	case (3): {
		err = bt_mesh_sensor_cli_get(&sensor_cli, NULL,
					     &bt_mesh_sensor_present_amb_light_level, NULL);
		if (err) {
	...
	case (6): {
		err = bt_mesh_sensor_cli_get(&sensor_cli, NULL, &bt_mesh_sensor_present_amb_temp,
					     NULL);
		if (err) {
			printk("Error getting sensor temperature (%d)\n", err);
		}
		break;
	}
	case (7): {
		err = bt_mesh_sensor_cli_get(&sensor_cli, NULL, &bt_mesh_sensor_pressure, NULL);
		if (err) {
			printk("Error getting sensor pressure (%d)\n", err);
		}
		break;
	}
	}
